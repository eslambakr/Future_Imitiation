...
class DataLoader:
    ...
    def get_batch(self, episode_num, item_nums):
        ...
        if self.episode_num != episode_num:
            ...
            self.measurements_per_episode_temp = np.load(self.config.labels_dir + '/' + self.measurements_files[episode_num])
            self.direction_per_episode_temp = np.load(self.config.direction_dir + '/' + self.measurements_files[episode_num])
        if self.config.preprocessed_data:
            for item_num in item_nums:
                self.forward_images.append(self.forward_temp[item_num])
                self.measurements_per_episode.append(self.measurements_per_episode_temp[item_num])
                self.direction_per_episode.append(self.direction_per_episode_temp[item_num])
            self.measurements_per_episode = np.asarray(self.measurements_per_episode)
            self.direction_per_episode = np.asarray(self.direction_per_episode)
            if self.config.separate_throttle_brake and not self.config.speed_input:
                return self.forward_images, self.measurements_per_episode[:, :3], self.direction_per_episode
            if self.config.separate_throttle_brake and self.config.speed_input:
                return self.forward_images, self.measurements_per_episode, self.direction_per_episode
            else:
                # TODO: cover this case {if not self.config.separate_throttle_brake and self.config.speed_input}
                return self.forward_images, self.measurements_per_episode[:, :2], self.direction_per_episode
        else:
            for item_num in item_nums:
